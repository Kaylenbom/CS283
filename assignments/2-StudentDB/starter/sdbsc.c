...
int get_student(int fd, int id, student_t *s){
    ...
    // Get the offset of the student location
    int offset = id * STUDENT_RECORD_SIZE;
    if (lseek(fd, offset, SEEK_SET) == -1){
        return ERR_DB_FILE;
    }

    // We read in the if found
     ssize_t read_student_record = read(fd, s, STUDENT_RECORD_SIZE);

     if (read_student_record == -1){
        return ERR_DB_FILE;
     } else if(read_student_record < STUDENT_RECORD_SIZE){
...
int add_student(int fd, int id, char *fname, char *lname, int gpa){
    ...
    student_t temp;
    ssize_t read_student_record = read(fd, &temp, STUDENT_RECORD_SIZE);
    ...
    // Checking if at the location, if the student record is not an empty student record, then a student already exists
    if (temp.id != 0 && temp.fname[0] != '\0' && temp.lname[0] != '\0' && temp.gpa != 0) {
    ...
    // write the new students information in their location
...
int del_student(int fd, int id){
    ...
    // checking the result of the get_student
    ...
    // find the location of the student to delete 
    int offset = id * STUDENT_RECORD_SIZE;
    ...
    // write an empty record essentially "deleting" the student record
    ssize_t empty_record = write(fd, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE);

    if(empty_record != STUDENT_RECORD_SIZE){
        printf(M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    }
...
int count_db_records(int fd){
    ...
    while(1){
        ...
        ssize_t read_student_record = read(fd, &student, STUDENT_RECORD_SIZE);

        // check for EOF
        if(read_student_record == 0){
            break;
        }
        
        // Check for if its an empty record and dont count it
...
int print_db(int fd){
    ...
    // a flag to check for an empty databse
    int empty = 1;

    student_t student;
    // a flag to check if header is printed
    int header = 0;
    ...
    while(1){
        ...
        // Check for if its an empty record
        if(memcmp(&student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) == 0){
            id++;
            offset = id * STUDENT_RECORD_SIZE;
            continue;
